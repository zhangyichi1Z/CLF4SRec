...
class SequentialDataLoader(AbstractDataLoader):
    ...
    def _get_processed_data(self, index):
        ...
        elif self.config['SSL_AUG'] == 'CLF4SRec' and self.phase == 'train':
            self.myrec_aug(cur_data)
        elif self.config['SSL_AUG'] == 'MyRec4' and self.phase == 'train':
            self.duorec_aug(cur_data, index)

        # if self.config['noise'] == 'OPEN' and self.phase != 'train':
        #     print(cur_data['item_id_list'][0])
        if self.config['noise'] == 'OPEN' and self.phase != 'train':
            #print('noise')
            self.test_noise(cur_data,self.config['noise_r'])
        # if self.config['noise'] == 'OPEN' and self.phase != 'train':
        #     print(self.phase)
        #     print('lll')
        #     print(cur_data['item_id_list'][0])
    ...
    def test_noise(self, cur_data,noise_r):
        def item_mask(seq, length, gamma=noise_r):
            ...
            for index in mask_index:
                masked_item_seq[index] = random.randint(1,self.item_num)  # token 0 has been used for semantic masking
            return masked_item_seq, length
    ...
    def myrec_aug(self, cur_data):
        ...
        for seq, length in zip(seqs, lengths):
            ...
            aug_seq_r.append(aug_seq)
            aug_len_r.append(aug_len)

        cur_data.update(Interaction({'aug': torch.stack(aug_seq_r), 'aug_len': torch.stack(aug_len_r)}))
