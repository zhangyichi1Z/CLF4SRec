...
class Dataset(object):
    ...
    def split_by_ratio(self, ratios, group_by=None):
        ...
        # print(next_df[0]['user_id'][:30])
        # print(next_df[1]['user_id'][:30])
        # print(next_df[2]['user_id'][:30])
        next_ds = [self.copy(_) for _ in next_df]
        # print(next_ds)
        # print(next_ds[0]['user_id'][:30])
        # print(next_ds[1]['user_id'][:30])
        # print(next_ds[2]['user_id'][:30])
        # input()
        return next_ds
    ...
    def _split_index_by_leave_one_out(self, grouped_index, leave_one_num):
        ...
        for index in grouped_index:
            ...
            pr_r = math.ceil((tot_cnt - legal_leave_one_num) * self.config['train_r'])
            next_index[0].extend(index[:pr_r])
